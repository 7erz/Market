		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == v.button[0]) {
				try {
					...
					int st = stmt.executeUpdate("INSERT INTO book values ('" + v.tf[0].getText() + "', '"
							+ v.tf[1].getText() + "', '" + v.tf[2].getText() + "', '" + v.tf[3].getText() + "')");
				...
				try {
					stmt = con.createStatement();
					int st = stmt.executeUpdate("UPDATE book set title = '" + v.tf[1].getText() + "' where id = '"
							+ v.tf[0].getText() + "';");
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
